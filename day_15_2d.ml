...
(* Méthode avec A* avec distance euclidienne *)
...
let extract = function
  | E -> None
  | N s -> Some (s.value, merge s.left s.right)
;;
...
let _ =
  ...
  and score = Array.make_matrix h w max_int
  and heur l c =
    (* euclidian distance *)
    ((h - 1 - l) * (h - 1 - l)) + ((w - 1 - c) * (w - 1 - c))
    |> float_of_int |> sqrt |> int_of_float
  in
  let heap = ref (insert (0, 0, 0) (heur 0 0) E) in
  try
    while true do
      ...
      | Some ((l, c, s), he) ->
          ...
          if s < score.(l).(c) then begin
            ...
            then
              heap :=
                insert
                  (l - 1, c, s + grid_score (l - 1) c)
                  (s + grid_score (l - 1) c + heur (l - 1) c)
                  !heap;
            if
              l < h - 1
              && s + grid_score (l + 1) c
            ...
            then
              heap :=
                insert
                  (l, c - 1, s + grid_score l (c - 1))
                  (s + grid_score l (c - 1) + heur l (c - 1))
                  !heap;
            if
              c < w - 1
              && s + grid_score l (c + 1)
