...
let _ =
  ...
  let scanner_pos = Array.make nb_scanners [||] in
  (* Graph traversal *)
  ...
  let rec visit s =
    for i = 0 to nb_scanners - 1 do
      ...
      then begin
        todo.(i) <- false;
        connect scanners processed_scanners s i scanner_pos;
        visit i
      end
    done
  in
  ...
  let man = ref 0 in
  for i = 1 to nb_scanners - 1 do
    for j = 0 to i - 1 do
      man :=
        max !man (manhattan scanner_pos.(i) scanner_pos.(j))
    done
  done;
  Printf.printf "Par Two: %d\n" !man
;;
